...
// adjusting size
integrated_button::integrated_button(wxWindow *parent, wxPoint xy, wxWindowID ID, wxString label, wxString default_num, wxSize size)
{
	// txt_box_point calculates textbox location relative to size of button
	button = new wxButton(parent, ID, label, xy, size, wxBU_TOP);
	wxPoint txt_box_point = wxPoint(xy.x + (size.x - text_size.x)/2, xy.y + (size.y-text_size.y-5));
	txt_box = new wxTextCtrl(parent, ID, default_num, txt_box_point, text_size);
}
